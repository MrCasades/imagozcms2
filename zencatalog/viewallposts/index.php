...
try
{
	$sql = 'SELECT posts.id AS postid, post, author.id AS authorid, posttitle, imghead, imgalt, postdate, authorname, category.id AS categoryid, categoryname FROM posts 
			...
			WHERE premoderation = "YES" ORDER BY postdate DESC LIMIT '.$shift.' ,'.$onPage;//Вверху самое последнее значение
...
try
{
	$sql = "SELECT count(*) AS all_articles FROM posts WHERE premoderation = 'YES'";
	$result = $pdo->query($sql);
}
