...
if (isset($_GET['add']))//Если есть переменная add выводится форма
{
	...
	{
		$authors_1[] = array('idauthor' => $row['idauthor'], 'authorname' => $row['authorname']);
	}
	...
	foreach ($result as $row)
	{
		$genres_1[] = array('idgenre' => $row['idgenre'], 'genrename' => $row['genrename']);
	}
	...
	foreach ($result as $row)
	{
		$publishingcompanys_1[] = array('idcompany' => $row['idcompany'], 'companyname' => $row['companyname'], 'selected' => FALSE);
	}
...
if (isset ($_POST['action']) && $_POST['action'] == 'Upd')
{
	...
	foreach ($result as $row)
	{
		$publishingcompanys_1[] = array('idcompany' => $row['idcompany'],'companyname' => $row['companyname'], 'selected' => in_array($row['idcompany'], $selectedCompany));
	}
